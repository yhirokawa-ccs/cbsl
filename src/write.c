...
static
cbsl_errors streaming_write_flush(cbsl_ctx* ctx, const void* data, uint64_t size)
{
...
cbsl_errors streaming_write_immediate(cbsl_ctx* ctx, const void* data, uint64_t size)
{
  ...
  CBSL_CHECK_COND_AND_RETURN(streaming_write_flush(ctx, data, size) == cbsl_success, cbsl_error);
